...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    // Log attendance for students, instructors, or admins
    ...
    $record_type = ucfirst(strtolower(trim($data['record_type'])));
    $status = ucfirst(strtolower(trim($data['status'])));
    $student_id = (int)$data['student_id'];
    $subject_id = (int)$data['subject_id'];
    
    if ($record_type !== 'Log-in' && $record_type !== 'Log-out') {
    ...
    if ($_SESSION['role'] === 'student') {
        $currentStudentProfileId = isset($_SESSION['profile']['id']) ? (int)$_SESSION['profile']['id'] : 0;
        if ($student_id !== $currentStudentProfileId) {
            sendResponse(false, 'Students can only log attendance for their own profile', null, 403);
        }
    }
