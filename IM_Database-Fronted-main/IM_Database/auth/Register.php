...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            // Start transaction with SQL Server
            ...
            if (!$role_row) {
                ...
                $insert_role_sql = "INSERT INTO dbo.roles (role) VALUES (?)";
                $insert_role_stmt = sqlsrv_query($conn, $insert_role_sql, ['student']);
                if ($insert_role_stmt === false) {
                    throw new Exception('Failed to create missing student role');
                }
                $scope_stmt = sqlsrv_query($conn, "SELECT SCOPE_IDENTITY() AS id");
                if ($scope_stmt === false) {
                    throw new Exception('Failed to get inserted role ID');
                }
                $scope_row = sqlsrv_fetch_array($scope_stmt, SQLSRV_FETCH_ASSOC);
            ...
            $user_stmt = sqlsrv_query($conn, $user_sql, [$form_data['username'], $password_hash, $role_id, $account_code]);
            if ($user_stmt === false) {
            ...
            $id_stmt = sqlsrv_query($conn, $user_id_sql);
            if ($id_stmt === false) {
            ...
            $profile_sql = "INSERT INTO dbo.student_profiles 
                            ...
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $profile_stmt = sqlsrv_query($conn, $profile_sql, [
            ...
            if (!empty($transactionStarted) && sqlsrv_rollback($conn) === false) {
